    ...
    computed: {
      ...
      filterList () {
        return this.list.filter((item) => {
          ...
          const freedEpisodeSize = this.filter[this.getFilterItem('freedEpisodeSize')].active;
    ...
    methods: {
      ...
      observeScroll() {
        ...
        const getLastItem = (prevItem) => {
          ...
          const lastItem = items[items.length > 5 ? items.length - 5 : items.length - 1];
        ...
        const observerHandler = (entries) => {
          entries.forEach(async (entry) => {
            if (entry.isIntersecting) {
              ...
              this.page++;
              const { count, hasNext } = await this.getComicList({ genre: this.genre, page: this.page });
              this.count = count;
              this.hasNext = hasNext;
